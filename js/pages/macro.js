...
// ------------------------------------------------------------------
// SVG sparkline (plain SVG — no LightweightCharts for inline cards)
// ------------------------------------------------------------------
...
function renderSparklineSvg(svgEl, pts, maPoints, color) {
  ...
  if (recentPts.length < 2) { svgEl.innerHTML = ''; return; }
...
function renderStackedSparkline(svgEl, assetsData) {
  ...
  for (const s of normalized) {
    const d = s.pts.map(([t, v], i) => `${i === 0 ? 'M' : 'L'}${sx(t).toFixed(1)},${sy(v).toFixed(1)}`).join(' ');
...
function renderAssetCard(assetData, color, maPeriod) {
  ...
  if (above_ma === true)       { signalLabel = `ABOVE ${maPeriod}-DAY MA`; signalClass = 'above'; }
  else if (above_ma === false) { signalLabel = `BELOW ${maPeriod}-DAY MA`; signalClass = 'below'; }
  ...
  requestAnimationFrame(() => {
    ...
    if (svgEl && price_points?.length >= 2) {
      renderSparklineSvg(svgEl, price_points, ma_points || [], color);
    }
  });
  ...
  el.innerHTML = `
    ...
    <div class="regime-axes">
      <div class="regime-axis-group">
        ...
        <div class="regime-axis-detail">${renderAxisDetail(rc.growth.signals, 'Expansion', 'Contraction')}</div>
      </div>
      <div class="regime-axis-group">
        <div class="regime-axis">
        ...
        <div class="regime-axis-detail">${renderAxisDetail(rc.inflation.signals, 'Inflationary', 'Deflationary')}</div>
      </div>
    </div>
  `;
}
...
function renderRegimeFlagsCard(rc, signalMap) {
  ...
  el.innerHTML = `<div class="regime-card-title">Risk Signals</div><div class="regime-flag-list">${inner}</div>`;
}
...
function buildTabUI(categories) {
  ...
  const bar    = tabDefs.map(t => `<button class="tab-btn${t.id === activeTab ? ' active' : ''}" data-tab="${t.id}">${t.label}</button>`).join('');
  const panels = tabDefs.map(t => `<div class="tab-panel${t.id === activeTab ? ' active' : ''}" id="tab-${t.id}"></div>`).join('');
...
function renderOverviewTab(cache) {
  ...
  for (const catData of cache.categories) {
    ...
    const leadersHTML  = catData.leaders?.length   ? catData.leaders.map(s => chipWithDot(s, 'above')).join('')  : '<span class="sym-chip-none">—</span>';
    const laggardsHTML = catData.laggards?.length ? catData.laggards.map(s => chipWithDot(s, 'below')).join('') : '<span class="sym-chip-none">—</span>';
    ...
    card.innerHTML = `
      ...
      <div class="overview-chips-section">
        <div class="overview-chips-row"><span class="overview-chips-dir above">▲</span><div class="overview-chips">${leadersHTML}</div></div>
        <div class="overview-chips-row"><span class="overview-chips-dir below">▼</span><div class="overview-chips">${laggardsHTML}</div></div>
      </div>
...
function renderCategoryTab(catData, catConfig, maPeriod) {
  ...
  if (total > 0) {
    ...
    const barColor = pct >= 0.70 ? '#10b981' : pct >= 0.50 ? '#84cc16' : pct >= 0.30 ? '#f59e0b' : '#ef4444';
  ...
  for (const assetData of catData.assets) {
    assetsGrid.appendChild(renderAssetCard(assetData, catConfig.color, maPeriod));
  }
...
async function loadAndRender() {
  const cache = await fetchMacro(LOOKBACK_DAYS, MA_PERIOD);
  applyMacroCache(cache);
}
...
async function init() {
  ...
  try {
    ...
    const r = await fetch('./data/last_updated.txt', { cache: 'no-store' }).catch(() => null);
    if (r?.ok) {
